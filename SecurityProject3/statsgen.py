...
class StatsGen:
    ...
    def print_stats(self):
        ...
        print("    NOTE: Statistics below are relative to the number of analyzed passwords, not the total number of passwords")
...
if __name__ == "__main__":
    ...
    if not options.minlength is None: statsgen.minlength = options.minlength
    if not options.maxlength is None: statsgen.maxlength = options.maxlength
    if not options.charsets is None: statsgen.charsets = [x.strip() for x in options.charsets.split(',')]
    if not options.simplemasks is None: statsgen.simplemasks = [x.strip() for x in options.simplemasks.split(',')]
